...
var currGet = "",ORG= "",machines="",globalPath="",monitorFile="",monitorFTP="";
var cancelTrans = false,trigger_count=0;
var indervalId = "";
	var can = "",overlay="",curHead=0,
...
session(req,res,function(req,res){
	...
	if(req.session.UserSession !== undefined){
		res.write("<script type='text/javascript'>alert('Redirect');window.location='http://10.0.0.3:3000/postlogin/'</script>");
		res.end();
		return;
	}
	res.write(loginPage);
...
var ftp = new JSFTP({
	host:ORG.Machine[mMachine].host,
	port:21,
	user:"admin",
	pass:""
});
...
ftp.get("/SENDFILE.CFG",function(err,s){
	...
	if(err){
		...
		s.on("data",function(data){
			...
			ftp.get("/"+data.toString(),Repo+data.toString(),function(err){
				...
				else
				{
					...
					fs.readFile(Repo+monitorFile,function(err,lmk){
		if(err){
		...
		indervalId = setInterval(updateOverlay, 9000);
