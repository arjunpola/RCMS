...
var monitorFile="",monitorFTP="",curHead=0,ORG="";
...
session(req,res,function(req,res){
	...
	if(req.session.UserSession != undefined){
		res.write("<script type='text/javascript'>window.location='http://10.0.0.3:3000/postlogin/'</script>");
		res.end();
		return;
	}
		...
		if(err != null)
		{
			...
			if(pwd == rows[0].pwd){
				...
				res.end();
					...
					req.session.ORG = new Object();
					req.session.ORG.count = rows.length; 
					req.session.ORG.Machine = new Array();
...
app.get("/home",function(req,res)
		...
		session(req,res,function(req,res){
			...
			{
				res.write("<script>window.location='http://10.0.0.3:3000/login/'</script>");
				res.end();
			}
	});//Session
});
...
session(req,res,function(req,res){
	...
	var query = con.query('INSERT INTO looms.sailoomtech SET ?', post, function(err, result) {
	  ...
	  {
		  console.log("Mysql Query Error!");
		  res.write("<script type=text/javascript>alert(\"This machine ip already exists\");window.location='http://10.0.0.3:3000/addmachines/'</script>");
					res.end();
				//throw err; 
	  }
	  else
	  {
		  ...
		  res.write("<script type=text/javascript>alert(\"Machine added successfully\");window.location='http://10.0.0.3:3000/addmachines/'</script>");
		  res.end();
...
ftp.get("/SENDFILE.CFG",function(err,s){
	...
	if(err){
		...
		//throw err;
		res.write("<script>alert('Machine not connected.');window.location='http://10.0.0.3:3000/monitor/'</script>");
		res.end();
		}
		...
		if(err){
			...
			res.write("<script>alert(.Memory Full. Please Delete some Files.);window.location='http://10.0.0.3:3000/monitor/'</script>");
			res.end();
			}
		...
		if((No_of_Legs * Size_of_Leg * LC) != No_Of_Hooks)
		{
			...
			res.write("<script>alert('InValid LMKFile...'); window.location='http://10.0.0.3:3000/postlogin/'; </script>");
			res.end();	
			return;
		}
		...
		res.write("<script>window.location='http://10.0.0.3:3000/MonitorPrint/'</script>");
		res.end();
...
res.writeHead(200,{"Content-Type":"text/html"});
	...
	if(req.files.userPhoto.name == ""){
		 res.write("<script>alert('Please Select Valid File!'); window.location='http://10.0.0.3:3000/home/'</script>");
	     res.end();
		}
	else if(req.body.Machine === undefined){
		
		res.write("<script>alert('Please Select atleast 1 Machine'); window.location='http://10.0.0.3:3000/home/'</script>");
	    res.end();
	...
	req.session.machines = req.body.Machine;
	

	for(i=0;i<req.session.machines.length;i++)
	{
		console.log("Sending To Machine "+req.session.ORG.Machine[req.session.machines[i]].name);
	}
	
    req.session.globalPath = Repo + fileName;
    ...
    require('fs').rename(
        ...
        function(error){
            ...
            if(error){
			  ...
			  res.write("<script>alert('OOPS! Theres some problem. Please try again later.'); window.location='http://10.0.0.3:3000/home/'</script>");
			  res.end();	  	
/*               socketServer.socket.emit("TransRes",{Data:"OOPS! Theres some proble. Please Try Again!"}); */
        }
        
			
			res.write("<script>window.location='http://10.0.0.3:3000/uploading/'</script>");
    	...
    	pass:""
			...
			transFtp.put(req.session.globalPath,"/"+fileName,function(err){
				...
				if(err){
					...
					res.write("<script>alert('OOPS! Theres some problem. Please try again later.'); window.location='http://10.0.0.3:3000/home/'</script>");						res.end();
				}
				else{
